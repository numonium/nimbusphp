...
/* ~EN: move var init to glboal scope */
var Redactor;
...
(function($){
	...
	Redactor = function(element, options)
	{
		// Element
		this.$el = $(element);
		...
		this.opts = $.extend({
			...
			modal_image: String() + 
				...
				'</div>' +
				'<form id="redactorInsertImageForm" class="_-forms--wysiwyg--edit--img" method="post" action="" enctype="multipart/form-data">' +
					'<div id="redactor_tab1" class="redactor_tab">' +
						'<input type="file" id="redactor_file" name="file" />' +
						
						/* ~EN: add image controls { */
						
						'<div class="_-forms--wysiwyg--edit--fieldset _-forms--wysiwyg--edit--img--align">' +
							'<h3>Align Image:</h3>' +
							'<label><input type="radio" class="_-input--radio" name="img[align]" value="l" /> Left</label>' +
							'<label><input type="radio" class="_-input--radio" name="img[align]" value="c" /> Center</label>' +
							'<label><input type="radio" class="_-input--radio" name="img[align]" value="r" /> Right</label>' +
						'</div>' +
						'<div class="_-forms--wysiwyg--edit--fieldset _-forms--wysiwyg--edit--img--size">' +
							'<h3>Image Size:</h3>' +
							'<label><input type="text" class="_-input--text short" name="img[w]" value="" /> Width (px)</label>' +
							'<label><input type="text" class="_-input--text short" name="img[h]" value="" /> Height (px)</label>' +
						'</div>' +
						'<div class="_-forms--wysiwyg--edit--fieldset _-forms--wysiwyg--edit--img--link">' +
							'<h3>Add Link:</h3>' +
							'<label><input type="text" class="_-input--text" name="img[link]" value="" /></label>' +
						'</div>' +
						'<div class="_-forms--wysiwyg--edit--fieldset _-forms--wysiwyg--edit--img--caption">' +
							'<h3>Add Caption:</h3>' +
							'<label><input type="text" class="_-input--text" name="img[caption]" value="" /></label>' +
						'</div>' +
						
						/* } add image controls :EN~ */
				...
				'<div id="redactor_modal_footer">' +
					...
					/* ~EN: add css classes {
						'<span class="redactor_btns_box _-forms--wysiwyg--edit--fieldset">' +						
							'<input type="button" name="upload" id="redactor_upload_btn" value="' + RLANG.insert + '" />' +
							'<a href="javascript:void(null);" id="redactor_btn_modal_close">' + RLANG.cancel + '</a>' + */
							
					'<span class="redactor_btns_box _-forms--wysiwyg--edit--fieldset">' +
						
						'<input type="button" class="_-input--submit" name="upload" id="redactor_upload_btn" value="' + RLANG.insert + '" />' +
						'<a href="javascript:;" id="redactor_btn_modal_close" class="_-input--reset">' + RLANG.cancel + '</a>' +
	...
	Redactor.prototype = {
		...
		{	
			...
			else if (this.opts.toolbar !== false)
			{
				...
				/* ~EN: add custom buttons *
				$.extend(this.opts.toolbar, this.opts.buttonsCustom); */
				$.extend(this.opts.toolbar, this.opts.buttonsCustom, _.ui.editors.wysiwyg.buttons);
			...
			/* ~EN: add to list of editors - if target element has no id, give it one */
			this._id = this.$el[0].id || _.str.uniqid('_-editor-');
			if(this.$el[0].id == '' || !this.$el[0].id){
				this.$el[0].setAttribute('id', this._id);
			}
			
			_.ui.editors[this._id] = this;
		...
		{	
			if (this.savedSel && this.savedSelObj !== null && this.savedSel[0].tagName !== 'BODY')
			{		
		...
		_imageSet: function(json, link)
		{
			...
			if (json !== false)
			{
				...
				if (link !== true)
				{
					...
					/* ~EN: add image attributes { */
					var attrs = '';
					
					if(data.img && data.img.width){
						attrs += 'width="' + data.img.width + '" ';
					}
					
					if(data.img && data.img.height){
						attrs += 'height="' + data.img.height + '" ';
					}
					
					
					/* } add image attributes :EN~ *
					
					html = '<p><img src="' + data.filelink + '" /></p>'; */
					
					/* ~EN: wrap image { */
					
					html = '<div class="_-img _-wysiwyg--img' + (data.img.align ? ' _-align--' + data.img.align : '') + (data.img.caption ? ' _-has--caption' : '') + '" contenteditable="false">' +
								'<img class="' + (data.img.align ? '_-align--' + data.img.align : '') + '" src="' + data.filelink + '" ' + attrs + ' />' +
								(data.img.caption ?
									'<span class="_-img--caption" contenteditable="true">' + data.img.caption + '</span>'
								 : '' ) +
							'</div>';
		...
		showLink: function()
		{
			...
			{			
				...
				{					
					...
					{
						/* ~EN: prefer "loc" attribute over href *
				...
				$('#redactor_insert_link_btn').live('click',$.proxy(this.insertLink, this));
		...
		insertLink: function()
		{
			...
			/* ~EN: prefer "loc" attr over "href" *
			this._insertLink('<a href="' + link + '" target="' + target + '">' +  text + '</a>&nbsp;', $.trim(text), link, target); */
			
			this._insertLink('<a href="javascript:;" loc="' + link + '">' +  text + '</a> ', $.trim(text), link, target); 
		...
		modalInit: function(title, url, width, handler, endCallback)
		{
			...
			{
				/* ~EN: add class */
				this.modal = $('<div id="redactor_modal" class="_-wysiwyg--modal" style="display: none;"><div id="redactor_modal_close">&times;</div><div id="redactor_modal_header"></div><div id="redactor_modal_inner"></div></div>');
		...
		uploadForm : function(f, name)
		{
			if (this.uploadOptions.input)
			{
				...
				/* ~EN: append additional fields from form { */
				
				if(f[0] && f[0].tagName && (f[0].tagName.toLowerCase() == 'form')){
					var inputs = $(f).find(':input'), _f = this.form, _ele = this.uploadOptions.input;
					
					if(inputs.length > 0){
						
						inputs.each(function(i){
						
							if(this == _ele[0]){ //file input is default field, handled below
								return;
							}
							
							var _clone = $(this).clone();
							
							if(_.array.isin(this.type, ['checkbox','radio'])){
							...
							_.log('800',this,this.checked,this.id,this.name,_clone.type,_clone.checked);
							
							// clone is appended to modal form
							$(this).before(_clone);
							
							// form field appended to upload form
							$(_f).append(this);
				...
				/* } append additional fields from form :EN~ */
